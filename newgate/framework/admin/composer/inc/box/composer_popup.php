...
<?php } 
if ($type == 'color') { ?>
		if (jQuery('#block_as_link').is(':checked')) {
			jQuery('#block_link').parent().parent().show();
			
			jQuery('#block_link_target').parent().parent().show();
		}
		
		
		jQuery('#block_as_link').bind('change', function () {
			if (jQuery(this).is(':checked')) {
				jQuery('#block_link').parent().parent().show();
				
				jQuery('#block_link_target').parent().parent().show();
			} else {
				jQuery('#block_link').parent().parent().hide();
				
				jQuery('#block_link_target').parent().parent().hide();
			}
		} );
<?php } ?>
} );
	...
	function insertShortcode() { 
		if (window.tinyMCE) {
			...
			if (fb_add_button.is(':checked')) {
				shortcode_tag += " button=\"true\" buttontext=\"" + fb_button_text + "\" buttonlink=\"" + fb_button_link + "\"";
	...
	function insertShortcode() { 
		if (window.tinyMCE) {
			...
			var shortcode_tag = '', 
				...
				block_as_link = jQuery('#block_as_link'),  
				block_link = jQuery('#block_link').val(), 
				block_link_target = jQuery('#block_link_target');
			...
			for (var i = 0, ilength = popup_tr_value.length; i < ilength; i += 1) {
				...
				if (popup_tr_value.eq(i).attr('aria-required') === 'true' && block_as_link.is(':checked')) {
					if (popup_tr_value.eq(i).val() === '' || popup_tr_value.eq(i).val() === ' ') {
						alert('<?php _e('Enter required fields!', 'cmsmasters'); ?>');
			...
			shortcode_tag += "[colored_block  bgcolor=\"" + cb_glow + "\"";
			
			if (block_as_link.is(':checked')) {
				shortcode_tag += " block_as_link=\"true\" block_link=\"" + block_link + "\"";
				
				if (block_link_target.is(':checked')) {
					shortcode_tag += " block_link_target=\"" + block_link_target.val() + "\"";
				}
			}
			
			
			
			shortcode_tag += "]" + ((jQuery('#wp-column_content-wrap').hasClass('tmce-active')) ? tinyMCE.get('column_content').getContent() : column_content) + "[/colored_block]";
		...
		<div class="media-item">
			...
			<?php } else if ($type == 'color') { ?>
					...
					<tr>
						...
						<td class="field" style="padding-top:10px;">
							<input type="text" value="<?php echo ($content != '' && $color['bgcolor']) ? $color['bgcolor'] : '#ff938a'; ?>" name="cb_glow" id="cb_glow" class="popup_tr_value my-color-field" aria-required="true" data-default-color="#ff938a" />
						</td>
					</tr>
					<tr>
						<th class="label" valign="top" style="width:130px; padding-top:15px;" scope="row">
							<span class="alignleft">
								<label for="block_as_link"><?php _e('Link', 'cmsmasters'); ?></label>
							</span>
						</th>
						<td class="field" style="padding-top:15px;">
							<input type="checkbox" value="true" name="block_as_link" id="block_as_link"<?php echo ($content != '') ? (($color['block_as_link'] && $color['block_as_link'] == 'true') ? ' checked="checked"' : '') : ''; ?> />
							<label for="block_as_link"><?php _e('Add Link', 'cmsmasters'); ?></label>
						</td>
					</tr>
					<tr style="display:none;">
						<th class="label" valign="top" style="width:130px; padding-top:15px;" scope="row">
							<span class="alignleft">
								<label for="block_link"><?php _e('Block Link', 'cmsmasters'); ?></label>
							</span>
							<span class="alignright">
								<abbr class="required" title="<?php _e('required', 'cmsmasters'); ?>">*</abbr>
							</span>
						...
						<td class="field" style="padding-top:10px;">
							<input type="text" value="<?php echo ($content != '' && $color['block_link']) ? $color['block_link'] : ''; ?>" name="block_link" id="block_link" class="popup_tr_value" aria-required="true" />
						</td>
					</tr>
					<tr style="display:none;">
						<th class="label" valign="top" style="width:130px; padding-top:15px;" scope="row">
							<span class="alignleft">
								<label for="block_link_target"><?php _e('Link Target', 'cmsmasters'); ?></label>
							</span>
						</th>
						<td class="field" style="padding-top:15px;">
							<input type="checkbox" value="_blank" name="block_link_target" id="block_link_target"<?php echo ($content != '') ? (($color['target'] && $color['target'] == '_blank') ? ' checked="checked"' : '') : ''; ?> />
							<label for="block_link_target"><?php _e('Open link in a new tab/window', 'cmsmasters'); ?></label>
						</td>
					</tr>
			<?php } else { ?>
					<tr>
