...
if (
	...
	$cmsms_heading_bg_col = get_post_meta(get_the_ID(), 'cmsms_heading_bg_col', true);
	$cmsms_heading_bg_img = get_post_meta(get_the_ID(), 'cmsms_heading_bg_img', true);
...
<!-- Typekit (Futura) -->
<script src="https://use.typekit.net/xkm4ttp.js"></script>
...
<?php
 ...
 ?>
<body <?php body_class($cmsms_boxed); ?>>
<?php 
	get_template_part('inc/content','geoip-standard-alert');
?>
			...
			<li>
				...
				<a href="#" id="active-lang">
					...
					foreach ( $languages as $lang ){
						...
						echo '<img class="iclflag" src="'.$lang["country_flag_url"].'" alt="'.$lang["language_code"].'" title="'.$lang["native_name"].'" height="12" width="18">';
				...
				<ul class="sub-menu submenu-languages">
					...
					foreach ( $languages as $lang ){
						...
						echo '<li class="menu-item menu-item-language">';
	...
	<?php
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_text_logo']) {
			if ($cmsms_option[CMSMS_SHORTNAME . '_text_logo_title'] !== '') {
				...
				$blog_title = (get_bloginfo('name')) ? get_bloginfo('name') : 'Newgate';
			}
			...
			if ( get_field('logo', 'options') ) {
				echo '<a href="' . home_url() . '/" title="' . get_bloginfo('name') . '" class="logo">' . "\n\t" .
					'<img src="' . get_field('logo', 'options') . '" alt="' . get_bloginfo('name') . '" />' . "\r" .
				'</a>' . "\n";
			}
		}
	?>
